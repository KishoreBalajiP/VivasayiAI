...
- **Agricultural Loss Claim (`/claims/*`):** *(Phase 2 = implemented unless marked pending)*
  ...
  - **Audit (implemented for farmer actions):** append-only `ClaimAudit` on created/submitted/withdrawn/resubmitted **and** on every evidence mutation (`evidence_presigned` / `evidence_completed` / `evidence_deleted`) with `requestId` correlation; metadata never contains `s3Key`/bucket/owner fields; engine/admin actions join with the verification phases.
