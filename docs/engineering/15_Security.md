...
- **Agricultural Loss Claim (`/claims/*`):** *(Phase 2 = implemented unless marked pending)*
  - **Rate limits (implemented):** `claimLimiter` (`CLAIM_RATE_LIMIT_WINDOW_MS` / `CLAIM_RATE_LIMIT_MAX`, default 5 claims/hour per user) and `evidenceLimiter` (`EVIDENCE_RATE_LIMIT_WINDOW_MS` / `EVIDENCE_RATE_LIMIT_MAX`, default 20 uploads/hour per user), both keyed by the authenticated `cognitoSub`.
  - **Geometry validation (implemented):** GeoJSON Polygon only; exterior ring only (no holes MVP); server-side authoritative area calculation via spherical geometry (Phase 1 `parcelGeometry.service.js`). Overlap/remaining-eligibility checks (P5) are pending E9-S3/S4 and remain server-side.
  - **Evidence (implemented):** reuses the presigned S3 pipeline; MIME JPEG/PNG/WEBP only (magic-byte sniffed; the browser-declared MIME is never authoritative — both the stored S3 `Content-Type` and the byte signature must match the declared type); max `IMAGE_UPLOAD_MAX_BYTES` (enforced at presign with a 413 and re-measured server-side at complete); EXIF stripped via the existing normalization pipeline; owner-scoped keys under `claims/<claimId>/` (client can never supply an S3 key — the params schema only accepts `img_<uuid>`); signed GET URLs (5 min TTL) served to the claim owner only (admin arrives with the admin phase). **Phase 3:** complete is **atomic + idempotent** (status CAS `pending → processing`; one record per upload; repeated completes return stored metadata; missing object leaves the record `pending`). **pHash dedup on complete is pending E9-S6** (anti-fraud, explicitly not implemented).
  - **Idempotency (implemented):** owner-scoped unique `idempotencyKey` on create (compound sparse index); duplicate create returns the existing claim; duplicate submit returns the existing claim without a second audit entry; evidence complete is idempotent (atomic CAS prevents duplicate records).
  - **State guards (implemented):** centralized state machine (`claimState.service.js`, 10 states, terminal states immutable); evidence mutable only in `draft`, `submitted`, `more_evidence_required` (single source of truth in `claimEvidence.service.js`); resubmission only from `more_evidence_required` with configurable max/cooldown.
  - **Ownership (implemented):** all claim/evidence resources scoped by `cognitoSub`; foreign → 404; a foreign uploadId can never be attached/completed/deleted against the caller's claim; responses never expose `s3Key`/bucket names, `cognitoSub`, `profileId`, or `idempotencyKey`.
  - **AI boundary:** AI output validated against schema; acreage fields stripped; deterministic rule engine makes final decision (engine pending E9-S3/S4/S5). **No acreage is ever derived from images** (P4) — geometry + backend calculation only.
- **Phase 4 — AI evidence assessment (implemented, internal service only):** claim/owner-scoped — only claim-owned evidence in usable state (`stored`) reaches vision; foreign claim/evidence → 404 (IDOR-safe). Client-supplied `imageUrl`, `s3Key`, `cognitoSub`, and claim `state` are structurally ignored (only `{ claimId, cognitoSub, requestId }` are read). The model adapter is the existing shared Gemini instance (no new client/keys). The persistence boundary whitelists a frozen fields contract per image — acreage/polygon/compensation/approval/status output is structurally impossible to persist (server-side scrub, verified by tests). Uncertainty is first-class (`uncertain`, `confidence: unclear`, `imageQuality: unclear`, `null` = cannot determine). Idempotent + concurrency-safe (unique `claimassessment` row per claim + atomic status CAS; in-flight/completed-with-same-evidence reuse; changed evidence re-assesses on a new version boundary). Sanitized failures: provider/storage errors surface as `ApiError` internals only in logs; clients/tests get generic `Image storage unavailable` / `Claim evidence assessment failed`; no Gemini prompt/S3 key/bucket/owner/image bytes in any error or audit metadata (audit rows `ai_completed` / `assessment_failed`, actor `engine`). No public endpoint exposes this stage.
   - **Audit (implemented for farmer actions):** append-only `ClaimAudit` on created/submitted/withdrawn/resubmitted **and** on every evidence mutation (`evidence_presigned` / `evidence_completed` / `evidence_deleted`) with `requestId` correlation; metadata never contains `s3Key`/bucket/owner fields; engine/admin actions join with the verification phases (**Phase 4 adds `ai_completed` / `assessment_failed`**).
   - **Phase 5 — Deterministic Verification (E9-S5, implemented, internal service only):** no new public endpoint; the `verifyClaim` internal service runs the pure deterministic engine (`evaluateClaimVerification`) and the guarded orchestration (`verifyClaim` service). **No new secrets, keys, or external calls** are introduced — the engine uses existing config (`CLAIM_WINDOW_DAYS`, `CLAIM_AREA_OVERAGE_FRACTION`), the shared model adapter for the assessment, and the centralized state machine. **Security invariants:**
     - Engine actor is `"engine"` in all `ClaimAudit` entries (`verification_started`, decision rows); `fromState`/`toState` explicit; `requestId` correlated.
     - Audit metadata is hygienic: `{evidenceVersion, imageCount, overlapEvaluated:false, engineVersion}` — no `s3Key`/bucket/owner/prompt/URL/image bytes.
     - Caller arguments are strictly scoped: only `{ claimId, cognitoSub, requestId }` are read; extra fields (state, outcome, approvedAreaAcres, geometry, attackerCognitoSub, etc.) are structurally ignored (verified by tests P5-20).
     - AI guardrails preserved: the engine **never reads `claim.state`** (P5-50); AI-reported acres/polygon/compensation/approval are structurally ignored; AI acreage cannot rescue an out-of-parcel claim (P5-17, P5-49).
     - State machine: `submitted → processing` edge added to the frozen machine (documented in its header; `claimState.service.js` ALLOWED table + `claimState.test.js` updated) — this is the **only** legal inbound to `processing`, enabling the documented `draft → submitted → processing → <outcome>` flow for verification. No new states invented; terminal states remain terminal.
