...
- **Agricultural Loss Claim (`/claims/*`):** *(Phase 2 = implemented unless marked pending)*
  ...
  - **Evidence (implemented):** reuses the presigned S3 pipeline; MIME JPEG/PNG/WEBP only (magic-byte sniffed); max `IMAGE_UPLOAD_MAX_BYTES`; EXIF stripped via the existing normalization pipeline; owner-scoped keys under `claims/<claimId>/`; signed GET URLs (5 min TTL) served to the claim owner only (admin arrives with the admin phase). **pHash dedup on complete is pending E9-S6**.
  - **Idempotency (implemented):** owner-scoped unique `idempotencyKey` on create (compound sparse index); duplicate create returns the existing claim; duplicate submit returns the existing claim without a second audit entry.
  - **State guards (implemented):** centralized state machine (`claimState.service.js`, 10 states, terminal states immutable); evidence mutable only in `draft`, `submitted`, `more_evidence_required`; resubmission only from `more_evidence_required` with configurable max/cooldown.
  - **Ownership (implemented):** all claim/evidence resources scoped by `cognitoSub`; foreign → 404; responses never expose `s3Key`/bucket names, `cognitoSub`, `profileId`, or `idempotencyKey`.
  - **AI boundary:** AI output validated against schema; acreage fields stripped; deterministic rule engine makes final decision (engine pending E9-S3/S4/S5).
  - **Audit (implemented for farmer transitions):** append-only `ClaimAudit` on created/submitted/withdrawn/resubmitted with `requestId` correlation; engine/admin actions join with the verification phases.
