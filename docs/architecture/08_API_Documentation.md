...
> **Status (Phase 4 note — AI evidence assessment, E9-S4):** the public claim API surface is **unchanged** — no assessment endpoint was added (§18; the AI evidence stage is an **internal service** invoked by the future verification engine E9-S5, exercised directly by tests). The assessment writes to `claimassessment` (`status` lifecycle `pending → processing → completed | failed`, `version`, `model`, `evidenceVersion`, per-image frozen observations in `aiImageAssessments`, deterministic `aiAggregate`; decision fields stay `null`). It is claim/owner-scoped (foreign claim → 404), idempotent + concurrency-safe, versioned, and audited (`ai_completed` / `assessment_failed`, actor `engine`). Existing endpoints keep returning `assessment: null` from claim detail — the assessment is not part of the public claim payload yet. **Not implemented (explicit boundary):** no public assessment/verification endpoint, no final verification (E9-S5), no weather correlation, no pHash/fraud, no frontend/admin UI, no severity→acreage conversion (P4). 41-scenario Phase 4 suite added (13 vision + 28 assessment; full suite now 166).
