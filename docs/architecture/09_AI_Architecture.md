...
- **Left to product (unchanged by this work):** F-22's full "cause → treatment → safety → escalation" card schema, D-23 (retention/lifecycle — bucket choice and 90-day lifecycle; **signed-URL retrieval is implemented** via the ownership-checked, short-TTL `GET /upload/:uploadId/view`, consumed by the frontend to render persisted chat images after reload), D-24 (EXIF/consent — EXIF is stripped at upload as the approved normalize side-effect), D-38 (consent framework). `IMAGE_STORAGE_MODE=mock` / `IMAGE_AI_MODE=mock` are dev/test-only seams (documented in `.env.example`) for deterministic regression (<sup>t214</sup>); the live provider E2E is `t215-verify.mjs`.
